...
} from '../services/cotizacion-import.service';
import { ConfirmImportInput } from '../types/cotizacion-import.types';
...
/**
 * POST /api/cotizaciones/importar
 * Parsea y valida un archivo de cotización (PDF, Excel, imagen)
 */
export async function importarArchivo(req: AuthRequest, res: Response) {
  try {
    ...
    if (!solicitud_cotizacion_id) {
      return res.status(400).json({ error: 'ID de solicitud de cotización requerido' });
    }

    const preview = await importarYValidar(
      req.file.path,
      req.file.mimetype,
    ...
    const statusCode = error.statusCode || 500;
    console.error('Error al importar archivo de cotización:', error);
    res.status(statusCode).json({ error: error.message || 'Error al importar archivo de cotización' });
  }
}

/**
 * POST /api/cotizaciones/importar/confirmar
 * Confirma la importación y crea la cotización
 */
export async function confirmarImport(req: AuthRequest, res: Response) {
  try {
    const input: ConfirmImportInput = req.body;

    if (!input.solicitud_id || !input.solicitud_cotizacion_id || !input.proveedor_nombre) {
      return res.status(400).json({ error: 'Faltan campos requeridos' });
    }

    if (!input.archivo_path || !input.archivo_nombre) {
      return res.status(400).json({ error: 'Información del archivo requerida' });
    }
    ...
    const cotizacion = await confirmarImportacion(input, req.user?.id || null);
    res.status(201).json(cotizacion);
  } catch (error: any) {
    const statusCode = error.statusCode || 500;
