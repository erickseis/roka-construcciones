...
import { DollarSign, AlertCircle, Upload } from 'lucide-react';
import ImportarCotizacionVentaModal from './ImportarCotizacionVentaModal';
...
export default function RegistrarCotizacionVentaModal({ isOpen, onClose, solicitudCotizacionId, proveedor, items, solicitudId, onSuccess }: Props) {
  ...
  const calcTotal = () => {
    return items.reduce((sum, item) => {
      ...
      return sum + (precio * item.cantidad_requerida);
    }, 0);
  };
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {items.map((item) => {
                    ...
                    const subtotal = precio * item.cantidad_requerida;
                    return (
